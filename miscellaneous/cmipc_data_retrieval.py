...
def get_range_CMIPC_data_fileKeys(year, day, hour, minute, bandNum):
    ...
    if minute < MINUTE_INTERVAL or minute + MINUTE_INTERVAL > 59:
        ...
        lower_time = dt - delta
        lower_files = get_CMIPC_data_fileKeys(
            lower_time.year,
            lower_time.timetuple().tm_yday,
            lower_time.hour,
            bandNum,
            lower_time.minute,
            59,
        ...
        upper_files = get_CMIPC_data_fileKeys(
            upper_time.year,
            upper_time.timetuple().tm_yday,
            upper_time.hour,
            bandNum,
            0,
            upper_time.minute - 1,
    ...
    # No over/underflow
    return get_CMIPC_data_fileKeys(
        year, day, hour, bandNum, minute - MINUTE_INTERVAL, minute + MINUTE_INTERVAL
    )
    ...
    if "Contents" in response:
        ...
        for object in response["Contents"]:
            ...
            if f"C{bandNum:02}" in fileKey:
                ...
                startMinIdx = (
                    fileKey.index(SCAN_START_INDICATOR)
                    + SCAN_START_INDICATOR_LENGTH
                    + START_MINUTE_OFFSET
                )

                if (
                    min_minute
                    <= int(fileKey[startMinIdx : startMinIdx + MINUTE_FIELD_WIDTH])
                    <= max_minute
                ):
